...
class DatabaseOld(_DatabaseReminders, _DatabaseComments, _DatabaseKeystore, _DatabaseSubreddits, _DatabaseUserSettings):
	tables = {
		'reminders': '''
	...
	def __init__(self, debug=False, publish=False, clone=False):
		log.info(f"Initializing database_old class: debug={debug} publish={publish} clone={clone}")
		self.debug = debug
		self.dbConn = None
	...
	def init(self, debug, publish, clone):
		...
		if publish:
			for table in DatabaseOld.tables:
				c.execute(f"DROP TABLE IF EXISTS {table}")

		for table in DatabaseOld.tables:
			c.execute(DatabaseOld.tables[table])
	...
	def close(self, silent=False):
		if not silent:
			log.debug("Closing database_old")
		self.dbConn.commit()
		self.dbConn.close()

	def backup(self):
		log.info("Backing up database_old")
		self.close(True)
